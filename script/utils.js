...
export const  createTable=(data, itemsPerPage = 10, currentPage = 1)=> {
    const totalItems = data.length;
    console.log(data.length,'data lengthñ')
    const totalPages = Math.ceil(totalItems / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = Math.min(startIndex + itemsPerPage, totalItems);
    const paginatedData = data.slice(startIndex, endIndex);
    ...
    const tableRows =  paginatedData.map(el => `
        <tr>
            <td class="clickable-id" rowspan="${el.length}">${el.league.id}</td>
            ...
            <td class="season-dropdown" rowspan="${el.length}"><select>
                    ${el.seasons.map(option => `<option value="${option.year}">${option.year}</option>`).join('')}
                </select></td>
